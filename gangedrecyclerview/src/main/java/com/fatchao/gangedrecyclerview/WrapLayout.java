...
public class WrapLayout extends LinearLayout {
    ...
    private void initView() {
        Log.d("zlt", "childCount = " + getChildCount());
        if (getChildCount() != 2) {
            throw new IllegalArgumentException("WrapLayout must have only two childs!");
        }
    ...
    private void resetChildWidth(int targetWidth) {
        ...
        if (swapListener != null) {
            swapListener.onSwap(lChildTempW * 1.0f / lChildWidth);
        }
        requestLayout();
    }
