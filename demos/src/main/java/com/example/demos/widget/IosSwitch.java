...
/**
 * Created by 101900 on 2017/2/28.
 */
 ...
 * SwitchButton.
 */
public class IosSwitch extends View implements Checkable {
    ...
    public IosSwitch(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            typedArray = context.obtainStyledAttributes(attrs, R.styleable.IosSwitch);
        }

        shadowEffect = optBoolean(typedArray,
                R.styleable.IosSwitch_sb_shadow_effect,
                true);

        uncheckCircleColor = optColor(typedArray,
                R.styleable.IosSwitch_sb_uncheckcircle_color,
                0XffAAAAAA);//0XffAAAAAA;

        uncheckCircleWidth = optPixelSize(typedArray,
                R.styleable.IosSwitch_sb_uncheckcircle_width,
                dp2pxInt(1.5f));//dp2pxInt(1.5f);

        uncheckCircleOffsetX = dp2px(10);

        uncheckCircleRadius = optPixelSize(typedArray,
                R.styleable.IosSwitch_sb_uncheckcircle_radius,
                dp2px(4));//dp2px(4);
        ...
        shadowRadius = optPixelSize(typedArray,
                R.styleable.IosSwitch_sb_shadow_radius,
                dp2pxInt(2.5f));//dp2pxInt(2.5f);

        shadowOffset = optPixelSize(typedArray,
                R.styleable.IosSwitch_sb_shadow_offset,
                dp2pxInt(1.5f));//dp2pxInt(1.5f);

        shadowColor = optColor(typedArray,
                R.styleable.IosSwitch_sb_shadow_color,
                0X33000000);//0X33000000;

        uncheckColor = optColor(typedArray,
                R.styleable.IosSwitch_sb_uncheck_color,
                mBasePlaneColor);//0XffDDDDDD;

        checkedColor = optColor(typedArray,
                R.styleable.IosSwitch_sb_checked_color,
                mOpenSlotColor);//0Xff51d367;

        borderWidth = optPixelSize(typedArray,
                R.styleable.IosSwitch_sb_border_width,
                dp2pxInt(1));//dp2pxInt(1);

        checkLineColor = optColor(typedArray,
                R.styleable.IosSwitch_sb_checkline_color,
                Color.WHITE);//Color.WHITE;

        checkLineWidth = optPixelSize(typedArray,
                R.styleable.IosSwitch_sb_checkline_width,
                dp2pxInt(1f));//dp2pxInt(1.0f);

        checkLineLength = dp2px(6);

        int buttonColor = optColor(typedArray,
                R.styleable.IosSwitch_sb_button_color,
                Color.WHITE);//Color.WHITE;

        int effectDuration = optInt(typedArray,
                R.styleable.IosSwitch_sb_effect_duration,
                300);//300;

        isChecked = optBoolean(typedArray,
                R.styleable.IosSwitch_toggle,
                false);

        showIndicator = optBoolean(typedArray,
                R.styleable.IosSwitch_sb_show_indicator,
                false);

        background = optColor(typedArray,
                R.styleable.IosSwitch_sb_background,
                mBasePlaneColor);

        enableEffect = optBoolean(typedArray,
                R.styleable.IosSwitch_sb_enable_effect,
                true);
    ...
    private void toggle(boolean animate, boolean broadcast) {
        ...
        if (isEventBroadcast) {
            throw new RuntimeException("should NOT switch the state in method: [onSwitchChangeListener]!");
        }
        if (!isUiInited) {
            isChecked = !isChecked;
            if (broadcast) {
    ...
    private void broadcastEvent() {
        if (onToggleListener != null) {
            ...
            onToggleListener.onSwitchChangeListener(isChecked());
        }
    ...
    public interface OnToggleListener {
        void onSwitchChangeListener(boolean isChecked);
    }
